...
export const SecretDetectionCard: React.FC<SecretDetectionCardProps> = ({ secretIssues }) => {
  ...
  const handleGenerateSecureCode = async (secretId: string) => {
    ...
    try {
        ...
        suggestions.forEach((suggestion, index) => {
          if (suggestion.codeChanges && suggestion.codeChanges.length > 0) {
            const code = suggestion.codeChanges
              ...
              .join('\n');
            if (code.trim()) {
              generatedCodes.push(`// AI Solution ${index + 1} (${suggestion.confidence}% confidence)\n// ${suggestion.title}\n\n${code}`);
            }
          }
        });

        // Always add our fallback secure implementation as an alternative
      ...
      toast.error('Failed to generate secure code. Please try again.');
                        ...
                        <div className="mt-3 p-3 bg-purple-900/20 border border-purple-700/30 rounded">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="text-purple-400 text-sm">
                              <p>
                                <strong>AI-Generated Solution:</strong> This secure implementation was generated by AI based on your specific vulnerability context, programming language ({detectLanguageFromFilename(secret.filename)}), and security best practices.
                              </p>
                              <ul className="list-disc list-inside mt-2 text-xs space-y-1">
                                <li>Uses environment variables for secure secret storage</li>
                                <li>Includes proper error handling for missing secrets</li>
                                <li>Follows {detectLanguageFromFilename(secret.filename)} security conventions</li>
                                <li>Provides setup instructions for deployment</li>
                              </ul>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 p-3 bg-blue-900/20 border border-blue-700/30 rounded">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="text-blue-400 text-sm">
                              <p className="mb-2">
                                <strong>AI Analysis Preview:</strong> Click "Generate Secure Code" to get personalized fixes for:
                              </p>
                              <ul className="list-disc list-inside space-y-1 text-xs">
                                <li>Language: {detectLanguageFromFilename(secret.filename)}</li>
                                {detectFrameworkFromContext(secret.filename, secret.codeSnippet || '') && (
                                  <li>Framework: {detectFrameworkFromContext(secret.filename, secret.codeSnippet || '')}</li>
                                )}
                                <li>Vulnerability: {secret.type} in {secret.filename}</li>
                                <li>Context: {getEnhancedCodeContext(secret).split('\n').length} lines of code</li>
                              </ul>
                            </div>
                          </div>
                        </div>
